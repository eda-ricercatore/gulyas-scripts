...
"""
	...
	For a given set of LaTeX source files for a document, this script
	will find all citations from my BibTeX database by searching for
	the string "\cite{[BibTeX key]}", and variants of it.
	
	For each unique citation, it will store them in a set, and use
	it to search for the corresponding BibTeX entry in my BibTeX
	database, copy that BibTeX entry into a output file (another
	BibTeX file).
	...
	Motivations:
	+ Hide list of references (i.e., BibTeX entries) that are not
		used in the (working) LaTeX document.
	+ Provide a BibTeX file that contains a subset of BibTeX entries
		found in my BibTeX database.
"""
